...
internal object ExtensionRepositoryStore {
    ...
    private val defaultRepositories = listOf(
        ...
        InkuExtensionRepository(
            ...
            kind = InkuRepositoryKind.Manga,
            enabled = false,
            lastError = "Unavailable by default: the configured endpoint currently returns 404."
        )
    )
    ...
    suspend fun refreshRepository(
        ...
        runCatching {
            ...
            repository.copy(
                ...
                lastError = when {
                    repository.id == "anime-kohi-den" -> "Archived/unstable: $message"
                    repository.id == "manga-thepbone-revived" && "404" in message -> "Unavailable: HTTP 404 from configured endpoint."
                    else -> message
                }
            )
        }.also { updated ->
